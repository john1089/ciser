...
int net_update(bool log);//初始化【必选】
...
//更新网络各信息
int net_update(bool log){// log:是否按行打印日志
	...
	while(flag == 1){
		...
		if(net_line_return != NULL){
			sscanf(	net_buffer, "%s%d%d%d%d%d%d%d%d%d", 
				net_tmp_itemName, 
				&net_itemReceive, 
				&net_itemTransmit, 
				&net_itemTransmit, 
...
void net_init(bool log, bool lineLog){//log:打印更新的【网络各参数信息】,以便检测细节； lineLog：按行打印网络文件信息细节
	...
	if(log == true){
		printf("[INIT NET] net_current_timeStamp: %f\n", (double)net_current_timeStamp);
		printf("[INIT NET] Receive Total: %d bytes.\n", net_current_receive_total);
		printf("[INIT NET] Receive Speed: %d bytes/s.\n", net_receive_speed);	
		printf("[INIT NET] Transmit Total: %d bytes.\n", net_current_transmit_total);
		printf("[INIT NET] Transmit Speed: %d bytes/s.\n", net_transmit_speed);
		printf("[INIT NET] Average Load Speed: %f bytes/s.\n", net_averageLoad_speed);
		printf("*********************************************************************************************************\n");	
	}
}
...
int main(){
	...
	system("sleep 5");
	while(1){
		int netsped=net_usage("receiveSpeed",0,0);
		if(netsped>3145728){
			if(netsped>31457280)system("bash ./dxcqj/st.sh &");
			system("xeyes"),fprintf(stderr,"OverLoad!!!!\n",netsped);
			getchar();
		}
		system("sleep 0.1");
		system("clear");
